...
_FLASHINFER_KERNEL_FALLBACK_WARNED = False
...
def _flashinfer_sampling_ops_available(need_min_p_sampling: bool) -> bool:
    if need_min_p_sampling:
        return all(
            _has_sgl_kernel_op(op)
            for op in (
                "top_k_renorm_probs",
                "top_p_renorm_probs",
                "min_p_sampling_from_probs",
    ...
    return _has_sgl_kernel_op("top_k_top_p_sampling_from_probs")
        ...
        if sampling_info.is_all_greedy:
            ...
            if can_sample_directly_from_probs:
                ...
                if get_global_server_args().sampling_backend == "flashinfer":
                    if _flashinfer_sampling_ops_available(
                        ...
                        global _FLASHINFER_KERNEL_FALLBACK_WARNED
                        if not _FLASHINFER_KERNEL_FALLBACK_WARNED:
                            logger.warning(
                                "FlashInfer sampling kernels are unavailable in sgl_kernel. "
                                "Falling back to PyTorch sampling backend."
                            )
                            _FLASHINFER_KERNEL_FALLBACK_WARNED = True
