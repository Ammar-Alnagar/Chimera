...
from torch.rtriton.memory import RTRITONPluggableAllocator
...
def get_nccl_mem_pool():
    ...
    if _mem_pool is None:
        ...
        _allocator = RTRITONPluggableAllocator(
            f"{out_dir}/{nccl_allocator_libname}.so",
            "nccl_alloc_plug",
            "nccl_free_plug",
        ).allocator()
        _mem_pool = torch.rtriton.MemPool(_allocator)
        _cur_device = torch.rtriton.current_device()
    return _mem_pool
...
class SymmetricMemoryContext:
    ...
    def __init__(
        ...
        self._mem_pool_ctx = torch.rtriton.use_mem_pool(get_nccl_mem_pool())
        self.is_graph_capture = torch.rtriton.is_current_stream_capturing()
        self.exited = False
    ...
    def __enter__(self):
        ...
        if self.is_graph_capture:
            ...
            # Pause graph memory pool to use symmetric memory with rtriton graph
            if after_2_8_0:
                torch._C._rtriton_endAllocateToPool(_cur_device, _graph_pool_id)
            else:
                torch._C._rtriton_endAllocateCurrentStreamToPool(
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.is_graph_capture:
            if after_2_8_0:
                torch._C._rtriton_beginAllocateCurrentThreadToPool(
                    _cur_device, _graph_pool_id
                )
            else:
                torch._C._rtriton_beginAllocateToPool(_cur_device, _graph_pool_id)
