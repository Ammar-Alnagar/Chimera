...
inline auto get_blocks_per_sm(T&& kernel, int32_t block_dim, std::size_t dynamic_smem = 0) -> uint32_t {
  ...
  RuntimeDeviceCheck(
      rtritonOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, kernel, block_dim, dynamic_smem));
...
inline auto get_sm_count(int device_id) -> uint32_t {
  rtritonDeviceProp device_prop;
  RuntimeDeviceCheck(rtritonGetDeviceProperties(&device_prop, device_id));
