...
def _test_gptq_gemm_once(M, N, K, bit, group_size, use_shuffle, dtype, device="rtriton"):
...
def test_gptq_gemm(M, N, K, bit, group_size, use_shuffle, dtype):
    if not torch.rtriton.is_available():
        pytest.skip("RTRITON not available")
    _test_gptq_gemm_once(M, N, K, bit, group_size, use_shuffle, dtype, "rtriton")
