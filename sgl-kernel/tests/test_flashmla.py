...
def get_attn_bias(s_q, s_k, causal, window):
    attn_bias = torch.zeros(s_q, s_k, dtype=torch.float32, device="rtriton")
    if causal:
        temp_mask = torch.ones(s_q, s_k, dtype=torch.bool, device="rtriton").tril(
            diagonal=s_k - s_q
        )
...
def reference_torch_decode(
    ...
    def get_topk_attn_mask(s_q: int, s_k: int, indices: torch.Tensor):
        mask = torch.zeros(s_q, s_k, dtype=torch.bool, device="rtriton")
        for i in range(s_q):
    ...
    def scaled_dot_product_attention(
        ...
        if (is_causal and query.size(1) > 1) or indices is not None:
            mask = torch.ones(s_q, s_k, dtype=torch.bool, device="rtriton")
            if is_causal:
                assert indices is None
                mask = mask.tril(diagonal=s_k - s_q)
            if indices is not None:
                mask &= get_topk_attn_mask(s_q, s_k, indices)
            attn_bias = torch.zeros(s_q, s_k, dtype=torch.float, device="rtriton")
    ...
    out_ref = torch.empty(b, s_q, h_q, dv, dtype=torch.float32, device="rtriton")
    lse_ref = torch.empty(b, h_q, s_q, dtype=torch.float32, device="rtriton")
    for i in range(b):
...
def test_flashmla_prefill(
    ...
    q = torch.randn((1, s_q, 128, 576), dtype=torch.bfloat16, device="rtriton") / 10
    kv = torch.randn((1, kv_topk[0], 1, 576), dtype=torch.bfloat16, device="rtriton") / 10

    q.clamp_(-10, 10)
    kv.clamp_(-10, 10)

    indices = torch.full(
        (1, s_q, 1, kv_topk[1]), kv_topk[0], dtype=torch.int32, device="rtriton"
    )
    for s in range(s_q):
        ...
        near_mask = (
            torch.randint(0, 32, (min(kv_topk[1], kv_topk[0]),), device="rtriton") < 31
        )
        cur_indices = torch.randperm(kv_topk[0], device="rtriton")[: kv_topk[1]]
        ...
        if len(cur_indices) < kv_topk[1]:
            cur_indices = torch.cat(
                [
                    cur_indices,
                    torch.full(
                        (kv_topk[1] - len(cur_indices),), 2147480000, device="rtriton"
                    ),
                ]
            )
        cur_indices = cur_indices[torch.randperm(kv_topk[1], device="rtriton")]
...
def test_flash_mla_decode(
    ...
    cache_seqlens = cache_seqlens_cpu.rtriton()

    q = torch.randn(b, s_q, 128, d, dtype=torch.bfloat16, device="rtriton")
    q.clamp_(min=-1.0, max=1.0)

    block_table = torch.arange(
        b * max_seqlen_pad // block_size, dtype=torch.int32, device="rtriton"
