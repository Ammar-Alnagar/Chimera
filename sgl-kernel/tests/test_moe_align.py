...
def test_moe_align_block_size_compare_implementations(
    ...
    topk_ids = torch.argsort(torch.rand(num_tokens, num_experts, device="rtriton"), dim=1)[
        :, :topk
    ]
    ...
    sorted_ids_triton = torch.empty_like(sorted_ids_rtriton)
    sorted_ids_triton.fill_(topk_ids.numel())
    expert_ids_triton = torch.zeros_like(expert_ids_rtriton)
    ...
    assert torch.allclose(expert_ids_rtriton, expert_ids_triton, atol=0, rtol=0), (
        f"Expert IDs mismatch for block_size={block_size}, "
        f"num_tokens={num_tokens}, topk={topk}\n"
        f"RTRITON expert_ids: {expert_ids_rtriton}\n"
        f"Triton expert_ids: {expert_ids_triton}"
    )

    assert torch.allclose(
        num_tokens_post_pad_rtriton, num_tokens_post_pad_triton, atol=0, rtol=0
    ), (
        f"Num tokens post pad mismatch for block_size={block_size}, "
        f"num_tokens={num_tokens}, topk={topk}\n"
        f"RTRITON num_tokens_post_pad: {num_tokens_post_pad_rtriton}\n"
        f"Triton num_tokens_post_pad: {num_tokens_post_pad_triton}"
    )

    # Select an expert to check
    expert_idx = expert_ids_rtriton.max().item()

    # Get the first and last block id where expert_ids_rtriton == expert_idx
    matching_indices = torch.where(expert_ids_rtriton == expert_idx)[0]
    block_sorted_start = matching_indices[0].item() * block_size
    block_sorted_end = min(
        (matching_indices[-1].item() + 1) * block_size, num_tokens_post_pad_rtriton.item()
    )

    selected_sorted_ids_rtriton = sorted_ids_rtriton[
    ...
    assert torch.allclose(
        ...
        f"num_tokens={num_tokens}, topk={topk}\n"
        f"RTRITON sorted_ids: {selected_sorted_ids_rtriton}\n"
...
def test_moe_sum(m: int, topk: int, k: int, dtype: torch.dtype):
    input = torch.randn((m, topk, k), device="rtriton", dtype=dtype)
    actual = torch.empty((m, k), device="rtriton", dtype=dtype)
