...
def test_topk_kernel(bs: int, k: int, seq_len: int, has_row_starts: bool) -> None:
    ...
    torch.rtriton.set_stream(stream)
    score = torch.randn(bs, MAX_SEQ_LEN, dtype=torch.float32, device="rtriton")
    lengths = torch.full((bs,), seq_len, dtype=torch.int32, device="rtriton")

    if has_row_starts:
        row_starts = torch.randint(0, 2048, (bs,), dtype=torch.int32, device="rtriton")
    else:
        row_starts = None
...
def test_topk_transform_kernel(bs: int, k: int, seq_len: int, mode: str) -> None:
    ...
    cu_seqlens_q = torch.arange(
        0, num_tokens + 1, step=step, dtype=torch.int32, device="rtriton"
    )
    src_page_table = torch.arange(0, seq_len, dtype=torch.int32, device="rtriton")
...
def test_topk_transform_ragged_kernel(
    ...
    topk_indices_offset = torch.randint(
        0, 1024, (bs,), dtype=torch.int32, device="rtriton"
    )
