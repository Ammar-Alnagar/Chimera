...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.impl("all_reduce", torch::kRTRITON, &all_reduce);
  ...
  m.impl("apply_rope_pos_ids_cos_sin_cache", torch::kRTRITON, &apply_rope_pos_ids_cos_sin_cache);
  ...
  m.def(
      ...
      "int mult, int offset) -> ()");
  m.impl("downcast_fp8", torch::kRTRITON, &downcast_fp8);

  m.def("copy_to_gpu_no_ce(Tensor input, Tensor! output) -> ()");
  m.impl("copy_to_gpu_no_ce", torch::kRTRITON, &copy_to_gpu_no_ce);
  m.def("concat_mla_k(Tensor! k, Tensor k_nope, Tensor k_rope) -> ()");
  m.impl("concat_mla_k", torch::kRTRITON, &concat_mla_k);

  m.def("concat_mla_absorb_q(Tensor a, Tensor b, Tensor! out) -> ()");
  m.impl("concat_mla_absorb_q", torch::kRTRITON, &concat_mla_absorb_q);
  ...
  m.impl("sgl_per_token_group_quant_8bit", torch::kRTRITON, &sgl_per_token_group_quant_8bit);
  ...
  m.impl("sgl_per_token_group_quant_8bit_v2", torch::kRTRITON, &sgl_per_token_group_quant_8bit_v2);

  m.def("sgl_per_tensor_quant_fp8(Tensor input, Tensor output_q, Tensor output_s, bool is_static) -> ()");
  m.impl("sgl_per_tensor_quant_fp8", torch::kRTRITON, &sgl_per_tensor_quant_fp8);

  m.def("sgl_per_token_quant_fp8(Tensor input, Tensor output_q, Tensor output_s) -> ()");
  m.impl("sgl_per_token_quant_fp8", torch::kRTRITON, &sgl_per_token_quant_fp8);

  m.def(
      "tilelang_scaled_fp4_mm(Tensor! out, Tensor a, Tensor b,"
      "                      Tensor block_scale_a, Tensor block_scale_b,"
      "                      Tensor alpha) -> ()");
  m.impl("tilelang_scaled_fp4_mm", torch::kRTRITON, &tilelang_scaled_fp4_mm);

  m.def(
      "scaled_fp4_quant(Tensor! output, Tensor! input,"
      "                 Tensor! output_scale, Tensor! input_scale) -> ()");
  m.impl("scaled_fp4_quant", torch::kRTRITON, &scaled_fp4_quant);

  m.def("dsv3_fused_a_gemm(Tensor! output, Tensor mat_a, Tensor mat_b) -> ()");
  m.impl("dsv3_fused_a_gemm", torch::kRTRITON, &dsv3_fused_a_gemm);
  ...
  m.impl("scaled_fp4_experts_quant", torch::kRTRITON, &scaled_fp4_experts_quant);
  ...
  m.impl("silu_and_mul_scaled_fp4_experts_quant", torch::kRTRITON, &silu_and_mul_scaled_fp4_experts_quant);
  ...
  m.impl("tilelang_fp4_group_mm", torch::kRTRITON, &tilelang_fp4_group_mm);

  m.def("dsv3_router_gemm(Tensor! output, Tensor mat_a, Tensor mat_b) -> ()");
  m.impl("dsv3_router_gemm", torch::kRTRITON, &dsv3_router_gemm);
  ...
  m.impl("gptq_marlin_gemm", torch::kRTRITON, &gptq_marlin_gemm);
  ...
  m.impl("moe_sum_reduce", torch::kRTRITON, &moe_sum_reduce);

  m.def("moe_sum(Tensor input, Tensor! output) -> ()");
  m.impl("moe_sum", torch::kRTRITON, &moe_sum);

  m.def(
      "moe_fused_gate(Tensor input, Tensor bias, int num_expert_group, int topk_group, int topk, int "
      "num_fused_shared_experts, float routed_scaling_factor, bool apply_routed_scaling_factor_on_output) -> "
      "(Tensor[])");
  m.impl("moe_fused_gate", torch::kRTRITON, &moe_fused_gate);
  ...
  m.def(
      ...
      "(Tensor[])");
  m.impl("kimi_k2_moe_fused_gate", torch::kRTRITON, &kimi_k2_moe_fused_gate);
  ...
  m.impl("fp8_blockwise_scaled_grouped_mm", torch::kRTRITON, &fp8_blockwise_scaled_grouped_mm);
  ...
  m.def(
      ...
      "()");
  m.impl("prepare_moe_input", torch::kRTRITON, &prepare_moe_input);

  m.def("shuffle_rows(Tensor input, Tensor dst2src_map, Tensor output) -> ()");
  m.impl("shuffle_rows", torch::kRTRITON, &shuffle_rows);

  m.def("apply_shuffle_mul_sum(Tensor input, Tensor output, Tensor permutation, Tensor? factors) -> ()");
  m.impl("apply_shuffle_mul_sum", torch::kRTRITON, &apply_shuffle_mul_sum);
  ...
  m.def(
      ...
      "-> ()");
  m.impl("fused_qk_norm_rope", torch::kRTRITON, &fused_qk_norm_rope);
  ...
  m.def(
      ...
      "                        int n, int k) -> ()");
  m.impl("get_tilelang_w4a8_moe_mm_data", torch::kRTRITON, &get_tilelang_w4a8_moe_mm_data);
  ...
  m.impl("tilelang_w4a8_moe_mm", torch::kRTRITON, &tilelang_w4a8_moe_mm);
  ...
  m.impl("moe_wna16_marlin_gemm", torch::kRTRITON, &moe_wna16_marlin_gemm);
  ...
  m.def(
      ...
      "bool deterministic) -> ()");
  m.impl("tree_speculative_sampling_target_only", torch::kRTRITON, &tree_speculative_sampling_target_only);
  ...
  m.impl("reconstruct_indices_from_tree_mask", torch::kRTRITON, &reconstruct_indices_from_tree_mask);
  ...
  m.def(
      ...
      "int rtriton_stream) -> ()");
  m.impl("segment_packbits", torch::kRTRITON, &segment_packbits);
  ...
  m.impl("weak_ref_tensor", torch::kRTRITON, &weak_ref_tensor);
  ...
  m.impl("min_p_sampling_from_probs", torch::kRTRITON, &min_p_sampling_from_probs);

  m.def("top_k_renorm_probs(Tensor probs, Tensor! renorm_probs, Tensor? maybe_top_k_arr, int top_k_val) -> ()");
  m.impl("top_k_renorm_probs", torch::kRTRITON, &top_k_renorm_probs);

  m.def("top_p_renorm_probs(Tensor probs, Tensor! renorm_probs, Tensor? maybe_top_p_arr, float top_p_val) -> ()");
  m.impl("top_p_renorm_probs", torch::kRTRITON, &top_p_renorm_probs);
  ...
  m.impl("top_p_sampling_from_probs", torch::kRTRITON, &top_p_sampling_from_probs);
  ...
  m.impl("top_k_top_p_sampling_from_probs", torch::kRTRITON, &top_k_top_p_sampling_from_probs);

  m.def("top_k_mask_logits(Tensor logits, Tensor mask_logits, Tensor? maybe_top_k_arr, int top_k_val) -> ()");
  m.impl("top_k_mask_logits", torch::kRTRITON, &top_k_mask_logits);
  ...
  m.def(
      ...
      "-> Tensor[]");
  m.impl("fwd_sparse", torch::kRTRITON, &flash::mha_fwd_sparse);
  ...
  m.impl("varlen_fwd_sparse", torch::kRTRITON, &flash::mha_varlen_fwd_sparse);
  ...
  m.def(
      ...
      "   bool causal) -> ()");
  m.impl("convert_vertical_slash_indexes", torch::kRTRITON, &convert_vertical_slash_indexes);
  ...
  m.def(
      ...
      "   int context_size, int block_size_M, int block_size_N, "
      "   bool causal) -> ()");
  m.impl("convert_vertical_slash_indexes_mergehead", torch::kRTRITON, &convert_vertical_slash_indexes_mergehead);
  ...
  m.impl("qserve_w4a8_per_chn_gemm", torch::kRTRITON, &qserve_w4a8_per_chn_gemm);
  ...
  m.impl("qserve_w4a8_per_group_gemm", torch::kRTRITON, &qserve_w4a8_per_group_gemm);
  ...
  m.def(
      ...
      "dtype) -> Tensor");
  m.impl("ggml_dequantize", torch::kRTRITON, &ggml_dequantize);

  m.def(
      "ggml_mul_mat_vec_a8(Tensor W, Tensor X, int type, SymInt row) "
      "-> Tensor");
  m.impl("ggml_mul_mat_vec_a8", torch::kRTRITON, &ggml_mul_mat_vec_a8);

  m.def("ggml_mul_mat_a8(Tensor W, Tensor X, int type, SymInt row) -> Tensor");
  m.impl("ggml_mul_mat_a8", torch::kRTRITON, &ggml_mul_mat_a8);
  ...
  m.impl("ggml_moe_a8", torch::kRTRITON, &ggml_moe_a8);

  m.def(
      "ggml_moe_a8_vec(Tensor X, Tensor W, "
      "Tensor topk_ids, int top_k, "
      "int type, SymInt row, SymInt tokens) -> Tensor");
  m.impl("ggml_moe_a8_vec", torch::kRTRITON, &ggml_moe_a8_vec);

  m.def("ggml_moe_get_block_size(int type) -> int");
  m.impl("ggml_moe_get_block_size", torch::kRTRITON, &ggml_moe_get_block_size);
  ...
  m.def(
      ...
      "int pad_slot_id) -> ()");
  m.impl("causal_conv1d_update", torch::kRTRITON, &causal_conv1d_update);
  ...
  m.def(
      ...
      "bool silu_activation,"
      "int pad_slot_id) -> ()");
  m.impl("causal_conv1d_fwd", torch::kRTRITON, &causal_conv1d_fwd);
  ...
  m.impl("fast_hadamard_transform", torch::kRTRITON, &fast_hadamard_transform);

  m.def("fast_hadamard_transform_12N(Tensor x, float scale) -> Tensor");
  m.impl("fast_hadamard_transform_12N", torch::kRTRITON, &fast_hadamard_transform_12N);

  m.def("fast_hadamard_transform_20N(Tensor x, float scale) -> Tensor");
  m.impl("fast_hadamard_transform_20N", torch::kRTRITON, &fast_hadamard_transform_20N);

  m.def("fast_hadamard_transform_28N(Tensor x, float scale) -> Tensor");
  m.impl("fast_hadamard_transform_28N", torch::kRTRITON, &fast_hadamard_transform_28N);

  m.def("fast_hadamard_transform_40N(Tensor x, float scale) -> Tensor");
  m.impl("fast_hadamard_transform_40N", torch::kRTRITON, &fast_hadamard_transform_40N);
  ...
  m.impl("timestep_embedding", torch::kRTRITON, &timestep_embedding);
}
