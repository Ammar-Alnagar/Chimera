...
rtritonError_t TreeSpeculativeSamplingTargetOnly(
    IdType* predicts,                   // mutable
  ...
  DISPATCH_ALIGNED_VEC_SIZE(
      vec_size, VEC_SIZE, {DISPATCH_DETERMINISTIC(deterministic, DETERMINISTIC, {
        auto kernel = TreeSpeculativeSamplingTargetOnly<
            ...
            IdType,
            IdType2>;
        FLASHINFER_RTRITON_CALL(rtritonFuncSetAttribute(kernel, rtritonFuncAttributeMaxDynamicSharedMemorySize, smem_size));
        FLASHINFER_RTRITON_CALL(rtritonLaunchKernel((void*)kernel, nblks, nthrs, args, smem_size, stream));
      })});
