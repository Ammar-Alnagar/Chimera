...
void tree_speculative_sampling_target_only(
  ...
  rtritonError_t status = sampling::TreeSpeculativeSamplingTargetOnly<float, int32_t, int64_t>(
  ...
  TORCH_CHECK(
      ...
      "TreeSpeculativeSamplingTargetOnly failed with error code " + std::string(rtritonGetErrorString(status)));
}
