...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.impl("get_mla_decoding_metadata", torch::kRTRITON, &get_mla_decoding_metadata);

  m.def("get_mla_decoding_metadata_dense_fp8(Tensor seqlens_k, int num_heads_per_head_k, int num_heads_k) -> Tensor[]");
  m.impl("get_mla_decoding_metadata_dense_fp8", torch::kRTRITON, &get_mla_decoding_metadata_dense_fp8);
  ...
  m.def(
      ...
      "-> Tensor[]");
  m.impl("fwd_kvcache_mla", torch::kRTRITON, &fwd_kvcache_mla);
  ...
  m.impl("dense_prefill_fwd", torch::kRTRITON, &FMHATileLangSM100FwdRun);

  m.def("sparse_prefill_fwd(Tensor q, Tensor kv, Tensor indices, float sm_scale, int d_v) -> Tensor[]");
  m.impl("sparse_prefill_fwd", torch::kRTRITON, &sparse_prefill_fwd);
  ...
  m.def(
      ...
      "descale_k) -> Tensor[]");
  m.impl("fwd_kvcache_mla_fp8", torch::kRTRITON, &fwd_kvcache_mla_fp8);
}
